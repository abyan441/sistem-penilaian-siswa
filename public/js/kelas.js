document.addEventListener("DOMContentLoaded", () => {
    ...
    /* =========================================================
       NOTIFIKASI
       Tidak mengubah style notifikasi yang sudah digunakan.
    ========================================================== */
    ...
    async function getJsonResponse(response) {
        ...
        try {
            ...
            throw new Error(
                "Server tidak mengembalikan data yang dapat diproses.",
            );
        }
    ...
    /* =========================================================
       VALIDASI DATA KELAS DARI SERVER
    ========================================================== */

    function validateClassData(data) {
        if (!data || typeof data !== "object") {
            throw new Error("Data kelas dari server tidak valid.");
        }
    ...
    function getCards() {
        return Array.from(
            document.querySelectorAll("#data-kelas [data-kelas-id]"),
        );
    }

    function getCardById(id) {
        return getCards().find(
            (card) => String(card.dataset.kelasId) === String(id),
        );
    }
    ...
    function classNameAlreadyUsed(name, exceptCard = null) {
    ...
    function waliAlreadyUsed(waliId, exceptCard = null) {
        return getCards().some((card) => {
            if (card === exceptCard) {
                return false;
            }

            return String(card.dataset.waliId || "") === String(waliId);
        });
    }
    ...
    function setWaliOptions() {
        ...
        Array.from(kelasWali.options).forEach((option, index) => {
            ...
            option.disabled = waliAlreadyUsed(option.value, editingCard);
        });
    }

    function getSelectedWaliName() {
        if (!kelasWali || kelasWali.selectedIndex < 0) {
            return "";
        }
    ...
    function updateSummary() {
        ...
        const totalSiswa = cards.reduce(
            (total, card) => total + getCardStudentCount(card),
            0,
        );
        ...
        const summaryValues = document.querySelectorAll(
            "#data-kelas .k-div-3 .k-div-4 .k-text-wrapper-4",
        );

        if (summaryValues[0]) {
            summaryValues[0].textContent = totalKelas;
        }

        if (summaryValues[1]) {
            summaryValues[1].textContent = totalSiswa;
        }

        if (summaryValues[2]) {
            summaryValues[2].textContent = rataRata;
        }
    }
    ...
    /* =========================================================
       BUAT CARD BARU
       Menggunakan class yang sama dengan Blade.
    ========================================================== */
    ...
    function createKelasCard(data) {
        ...
        if (!grid) {
            throw new Error(
                "Tempat untuk menampilkan data kelas tidak ditemukan.",
            );
        }
        ...
        card.innerHTML = `
            ...
            <nav
                aria-label="Aksi Kelas ${kelas.nama_kelas}"
                class="k-div-14"
            >

                <button
                    aria-label="Lihat detail Kelas ${kelas.nama_kelas}"
                ...
                <button
                    aria-label="Edit Kelas ${kelas.nama_kelas}"
    ...
    function updateKelasCard(card, data) {
        ...
        if (yearText) {
            yearText.textContent = `Tahun Ajaran ${kelas.tahun_ajaran}`;
        }

        if (infoTexts[0]) {
            infoTexts[0].textContent = kelas.wali_kelas;
        }

        if (infoTexts[1]) {
            infoTexts[1].textContent = `${kelas.jumlah_siswa} Siswa`;
        }
        ...
        if (levelContainer) {
            levelContainer.setAttribute("aria-label", `Tingkat ${newLevel}`);
        }

        if (nav) {
            nav.setAttribute("aria-label", `Aksi Kelas ${kelas.nama_kelas}`);
        }

        if (detailButton) {
            detailButton.setAttribute(
                "aria-label",
                `Lihat detail Kelas ${kelas.nama_kelas}`,
            );
        }
    ...
    function openKelasForm(isEdit) {
        ...
        if (kelasModalTitle) {
            kelasModalTitle.textContent = isEdit
                ? "Edit Data Kelas"
                : "Tambah Data Kelas";
        }

        if (kelasModalDesc) {
            kelasModalDesc.textContent = isEdit
                ? "Ubah nama kelas, tahun ajaran, atau wali kelas."
                : "Lengkapi data kelas yang akan ditambahkan.";
        }

        if (kelasSubmit) {
            kelasSubmit.textContent = isEdit
                ? "Simpan Perubahan"
                : "Simpan Data Kelas";
        }

        if (kelasDeleteButton) {
            kelasDeleteButton.hidden = !isEdit;
        }
    ...
    function fillEditForm(card) {
        ...
        if (kelasWali) {
            kelasWali.value = card.dataset.waliId || "";
        }
    ...
    kelasForm?.addEventListener("submit", async (event) => {
        ...
        if (classNameAlreadyUsed(name, editingCard)) {
            showMessage("Nama kelas sudah digunakan.");
        ...
        if (waliAlreadyUsed(waliId, editingCard)) {
            showMessage("Guru tersebut sudah menjadi wali kelas lain.");
        ...
        try {
            ...
            if (!result.success) {
                throw new Error(result.message || "Data kelas gagal diproses.");
            }

            const kelas = validateClassData(result.data);

            if (isEdit) {
                updateKelasCard(editingCard, kelas);
            } else {
                createKelasCard(kelas);
            }
            ...
            showMessage(
                error.message || "Terjadi kesalahan saat memproses data kelas.",
            );
        } finally {
            if (kelasSubmit) {
                kelasSubmit.disabled = false;
            }
        }
    });
    ...
    async function openDetail(card) {
        ...
        try {
            ...
            if (!result || !result.success || !result.data) {
                throw new Error("Data detail kelas dari server tidak valid.");
            }

            const data = result.data;

            if (data.id === undefined || data.nama_kelas === undefined) {
                throw new Error("Data detail kelas dari server tidak lengkap.");
            }

            if (detailNama) {
                detailNama.textContent = `Kelas ${data.nama_kelas}`;
            }

            if (detailTahun) {
                detailTahun.textContent = data.tahun_ajaran || "-";
            }

            if (detailWali) {
                detailWali.textContent = data.wali_kelas || "-";
            }

            if (detailJumlah) {
                detailJumlah.textContent = `${data.jumlah_siswa || 0} Siswa`;
            }

            if (detailStudentCount) {
                detailStudentCount.textContent = `${data.jumlah_siswa || 0} siswa`;
            }
            ...
            showMessage(
                error.message || "Gagal mengambil detail kelas dari database.",
            );
        }
    }
    ...
    function openDeleteConfirmation(card) {
        ...
        if (deleteConfirm) {
            deleteConfirm.disabled = jumlahSiswa > 0;
        }
    ...
    deleteConfirm?.addEventListener("click", async () => {
        if (!pendingDeleteCard || deleteConfirm.disabled) {
            return;
        }
        ...
        try {
            ...
            if (!result.success) {
                throw new Error(result.message || "Data kelas gagal dihapus.");
            }
            ...
            showMessage(
                result.message || `Kelas ${name} berhasil dihapus.`,
                "success",
            );
        } catch (error) {
    ...
    document.querySelectorAll("[data-kelas-modal-close]").forEach((element) => {
        element.addEventListener("click", closeKelasForm);
    });
    ...
    kelasDetailClose?.addEventListener("click", () => {
