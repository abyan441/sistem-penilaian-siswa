...
class Nilai extends Model
{
    ...
    {
        return Kelas::query()->where('wali_kelas_id', $guruId)
            ->orderByDesc('tahun_ajaran')->orderBy('nama_kelas')->get();
    }
    ...
    public static function pastikanSiswaDalamKelas($kelasId, Collection $siswaIds): void
    {
        ...
        if ($jumlahValid !== $siswaIds->count()) {
            throw new InvalidArgumentException('Terdapat siswa yang tidak sesuai dengan kelas wali Anda.');
        }
    }
    ...
    public static function hitungNilaiAkhir($nilaiTugas, $nilaiUts, $nilaiUas): float
    {
        ...
        return round(($nilaiTugas * .30) + ($nilaiUts * .30) + ($nilaiUas * .40), 2);
    }
    ...
    private static function formatSiswaDenganNilai(Collection $siswa, Collection $nilai): Collection
    {
        return $siswa->values()->map(function ($siswa, $index) use ($nilai) {
            ...
            return [
                'id' => $siswa->id, 'nisn' => $siswa->nisn, 'nama_siswa' => $siswa->nama_siswa,
                'nomor' => $index + 1, 'nilai_id' => $nilaiSiswa?->id,
                'nilai_tugas' => $tugas, 'nilai_uts' => $uts, 'nilai_uas' => $uas,
                'nilai_akhir' => $akhir, 'predikat' => static::predikat($akhir),
            ];
        });
    }
    ...
    public static function dataNilai($guruId, $semester, $mapelId): array
    {
        ...
        return [
            'kelas_id' => $kelas->id, 'kelas' => $kelas->nama_kelas, 'tahun_ajaran' => $kelas->tahun_ajaran,
            'semester' => (int) $semester, 'mapel_id' => (int) $mapelId,
    ...
    {
        static::pastikanSemester($semester);
        $kelasQuery = Kelas::query()->whereKey($kelasId);
        if ($tahunAjaran) $kelasQuery->where('tahun_ajaran', $tahunAjaran);
        $kelas = $kelasQuery->first();
        if (!$kelas) throw new InvalidArgumentException('Kelas dan tahun ajaran yang dipilih tidak ditemukan.');
        if (!MataPelajaran::query()->find($mapelId)) throw new InvalidArgumentException('Mata pelajaran tidak ditemukan.');

        $siswa = static::daftarSiswa($kelas->id);
        $nilai = static::query()->where('semester', $semester)->whereIn('siswa_id', $siswa->pluck('id'))
            ->whereHas('guruMapel', fn ($query) => $query->where('mapel_id', $mapelId))
            ->orderBy('id')->get()->keyBy('siswa_id');

        return [
            'kelas_id' => $kelas->id, 'kelas' => $kelas->nama_kelas, 'tahun_ajaran' => $kelas->tahun_ajaran,
            'semester' => (int) $semester, 'mapel_id' => (int) $mapelId, 'guru_mapel_id' => null,
            'siswa' => static::formatSiswaDenganNilai($siswa, $nilai)->all(),
        ];
    }
        ...
        DB::transaction(function () use ($dataNilai, $guruMapel, $semester) {
            foreach ($dataNilai as $data) {
                ...
                static::updateOrCreate(
                    ['siswa_id' => $data['siswa_id'], 'guru_mapel_id' => $guruMapel->id, 'semester' => $semester],
                    ['nilai_tugas' => $nilaiTugas, 'nilai_uts' => $nilaiUts, 'nilai_uas' => $nilaiUas, 'nilai_akhir' => static::hitungNilaiAkhir($nilaiTugas, $nilaiUts, $nilaiUas)]
                );
            }
        });
    }
    ...
    {
        $data = static::query()->join('siswa', 'siswa.id', '=', 'nilai.siswa_id')->join('kelas', 'kelas.id', '=', 'siswa.kelas_id')
            ->whereNotNull('nilai.nilai_akhir')->select('kelas.tahun_ajaran', DB::raw('AVG(nilai.nilai_akhir) as rata_rata'))
            ->groupBy('kelas.tahun_ajaran')->orderBy('kelas.tahun_ajaran')->get();
